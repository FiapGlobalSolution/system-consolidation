...
static void initializeClients()
{
  ...
  wifi_client.setTrustAnchors(&cert);
    if (az_result_failed(az_iot_hub_client_init(
      &client,
      az_span_create((uint8_t*)device_config.iothub_host, strlen(device_config.iothub_host)),
      az_span_create((uint8_t*)device_config.device_id, strlen(device_config.device_id)),
      &options)))
  {
  ...
  mqtt_client.setServer(device_config.iothub_host, port);
...
static int generateSasToken(char* sas_token, size_t size)
{
  ...
  // Base64-decode device key
    int base64_decoded_device_key_length
      = base64_decode_chars(
        device_config.device_key, strlen(device_config.device_key), base64_decoded_device_key);
...
{
  connectToWiFi();
  config_server_begin(&device_config);
  initializeTime();
  printCurrentTime();
  initializeClients();
...
static void sendTelemetry()
{
  ...
  mqtt_client.publish(telemetry_topic, getTelemetryPayload(device_config), false);
  Serial.println("OK");
  delay(100);
  digitalWrite(LED_PIN, LOW);
}
...
void setup()
{
  device_config_load(&device_config);
...
void loop()
{
  config_server_handle();

  if (config_server_should_reconnect())
  {
    config_server_clear_reconnect_flag();
  ...
  u8g2.drawStr(0,10, device_config.device_id);
  u8g2.drawStr(0,25, "------------------");
